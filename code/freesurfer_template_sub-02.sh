...
#SBATCH --job-name=freesurfer_template_sub-02.job
#SBATCH --output=./code/freesurfer_template_sub-02.out
#SBATCH --error=./code/freesurfer_template_sub-02.err
...
datalad containers-run -m 'freesurfer_sub-02/ses-*' -n bids-freesurfer --output . --input 'sourcedata/cneuromod.anat.gradcorrect/sub-02/ses-*/anat/*_T1w.nii.gz' --input 'sourcedata/cneuromod.anat.gradcorrect/sub-02/ses-*/anat/*_T2w.nii.gz' -- sourcedata/cneuromod.anat.gradcorrect ./ participant --steps template --refine_pial T2 --hires_mode enable --participant_label 02  --skip_bids_validator --license_file code/freesurfer.license --n_cpus 4 
freesurfer_exitcode=$?

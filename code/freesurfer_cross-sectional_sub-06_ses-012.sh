...
#SBATCH --job-name=freesurfer_cross-sectional_sub-06_ses-012.job
#SBATCH --output=./code/freesurfer_cross-sectional_sub-06_ses-012.out
#SBATCH --error=./code/freesurfer_cross-sectional_sub-06_ses-012.err
...
datalad containers-run -m 'fMRIPrep_sub-06/ses-012' -n bids-freesurfer --output . --input 'sourcedata/cneuromod.anat.gradcorrect/sub-06/ses-012/anat/*_T1w.nii.gz' --input 'sourcedata/cneuromod.anat.gradcorrect/sub-06/ses-012/anat/*_T2w.nii.gz' -- sourcedata/cneuromod.anat.gradcorrect ./ participant --steps cross-sectional --refine_pial T2 --hires_mode enable --participant_label 06 --session_label 012 --skip_bids_validator --license_file code/freesurfer.license --n_cpus 4 
freesurfer_exitcode=$?

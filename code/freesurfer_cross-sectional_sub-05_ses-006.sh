...
#SBATCH --job-name=freesurfer_cross-sectional_sub-05_ses-006.job
#SBATCH --output=/home/basile/data/tests/anat.freesurfer_longitudinal/code/freesurfer_cross-sectional_sub-05_ses-006.out
#SBATCH --error=/home/basile/data/tests/anat.freesurfer_longitudinal/code/freesurfer_cross-sectional_sub-05_ses-006.err
...
datalad containers-run -m 'fMRIPrep_sub-05/ses-006' -n containers/bids-freesurfer --output . --input 'sourcedata/cneuromod.anat.gradcorrect/sub-05/ses-006/anat/*_T1w.nii.gz' --input 'sourcedata/cneuromod.anat.gradcorrect/sub-05/ses-006/anat/*_T2w.nii.gz' --input 'sourcedata/cneuromod.anat.gradcorrect/sub-05/ses-006/anat/*_FLAIR.nii.gz' -- sourcedata/cneuromod.anat.gradcorrect ./ participant --steps cross-sectional --refine_pial --reconstruction_label norm --refine_pial_reconstruction_label norm --hires_mode enable --participant_label 05 --session_label 006 --skip_bids_validator --license_file code/freesurfer.license --n_cpus 4 
freesurfer_exitcode=$?

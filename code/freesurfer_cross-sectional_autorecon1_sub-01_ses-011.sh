...
#SBATCH --job-name=freesurfer_cross-sectional_autorecon1_sub-01_ses-011.job
#SBATCH --output=./code/freesurfer_cross-sectional_autorecon1_sub-01_ses-011.out
#SBATCH --error=./code/freesurfer_cross-sectional_autorecon1_sub-01_ses-011.err
...
datalad containers-run -m 'freesurfer_sub-01/ses-011' -n bids-freesurfer --output . --input 'sourcedata/cneuromod.anat.gradcorrect/sub-01/ses-011/anat/*_T1w.nii.gz' --input 'sourcedata/cneuromod.anat.gradcorrect/sub-01/ses-011/anat/*_T2w.nii.gz' --input 'sourcedata/cneuromod.anat.gradcorrect/sub-01/ses-011/anat/*_FLAIR.nii.gz' -- sourcedata/cneuromod.anat.gradcorrect ./ participant --steps cross-sectional --stages ['autorecon1'] --refine_pial T2 --hires_mode enable --participant_label 01 --session_label 011 --skip_bids_validator --license_file code/freesurfer.license --n_cpus 4 
freesurfer_exitcode=$?
